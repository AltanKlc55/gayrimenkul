...
<div class="row">
    <div class="col-xl-12">
        <div class="card custom-card">
            <form action="{{route('test.edit')}}" id="form" name="form" method="POST">
                @csrf
                        ...
                        <div class="col-md-8">
                            <div class="row">
                                ...
                                <input type="text" name="test_name" required class="form-control answer-text"
                                    value="{{$page['row']->test_name}}">
                                </div>
                                <input type="hidden" name="selected_values" id="selected_values" value="{{$page['row']->questions}}">
                                <div class="form-group mb-3 col-md-6">
                                    <label>Test Açıklaması</label>
                                    <input type="text" name="test_description" required
                                        class="form-control answer-value" value="{{$page['row']->test_description}}">
                                </div>
                                <input type="hidden" name="id" required class="form-control answer-value" value="{{$page['row']->id}}">
                                        ...
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-body">
                                                ...
                                                <table id="globaltable"
                                                    class="table globaltable table-striped  table-bordered text-nowrap w-100 dataTable no-footer"
                                                    ...
                                                    <tbody>
                                                        @foreach ($page['questions'] as $q)
                                                            <tr>
                                                                ...
                                                                <td>{{$q->question}}</td>
                                                                <td>{{$q->created_at}}</td>
                                                                <td><input class="form-check-input table_check"
                                        ...
                                        <div class="form-group-wrapper mb-3"
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="form-group mb-3 col-md-12">
                                                    ...
                                                    <input type="text" required class="form-control result_description"
                                                        value="{{$p->result_description}}">
                                                </div>
                                            </div>
                                            <button type="button" class="btn btn-danger btn-sm remove-group">Sil</button>
                                        </div>
                                    @endforeach
                                    <input type="hidden" required class="form-control" id="points_json"
                                        name="points_json" value="">
                                </div>
                                <div class="text-center">
                                    <button type="button" id="add-group" class="btn btn-primary mt-3">Puanlama
                                        Ekle</button>
                                </div>
                            </div>
                        </div>
...
@push('javascript')
    ...
    <script>
        $(document).ready(function () {
            ...
            var checked_values = [{{$page['row']->questions}}];

            $('.table_check').each(function () {
                var checkboxValue = $(this).val();
                if (checked_values.includes(parseInt(checkboxValue))) {
